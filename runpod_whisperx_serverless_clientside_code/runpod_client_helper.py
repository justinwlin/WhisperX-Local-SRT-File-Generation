...
def convert_to_mono_mp3(input_path, output_path):
    ...
    and writes it to the specified output file location.

    This function takes an input file path, converts the file to a mono MP3
    file with a bitrate of 32k, and writes this MP3 file to the specified
    output file location. It handles the creation and deletion of temporary
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as temp_mp3_file:
            ...
            (
                ...
                .output(temp_mp3_path, ac=1, ar="22050", ab="32k", format="mp3")
                .run(overwrite_output=True)
            )
            print(f"Conversion to MP3 successful. Temp file created at {temp_mp3_path}")

            # Move the temporary MP3 file to the specified output path
